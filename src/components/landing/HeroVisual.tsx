const KPIS = [
  { label: "العملاء النشطون", value: "1,248", delta: "+18%", w: "72%", tone: "cyan" as const },
  { label: "المهام المتأخرة", value: "8", delta: "-12%", w: "22%", tone: "warn" as const },
  { label: "الأتمتة", value: "24", delta: "+6", w: "60%", tone: "cyan" as const },
  { label: "التقارير الذكية", value: "36", delta: "+9", w: "84%", tone: "cyan" as const },
];
...
function FloatingBadge({ label, position }: { label: string; position: "top-right" | "bottom-left" | "middle-right" }) {
  ...
  return (
    <span
      aria-hidden="true"
      dir="ltr"
      className={`pointer-events-none absolute z-20 hidden md:inline-flex items-center gap-1.5 rounded-full border border-[rgba(34,211,238,0.34)] bg-[rgba(10,22,40,0.85)] backdrop-blur-md px-3 py-1.5 text-[11px] font-medium text-[#22D3EE] shadow-[0_8px_24px_-8px_rgba(34,211,238,0.45)] ${placement}`}
    >
...
function KpiCard({ label, value, delta, w, tone }: (typeof KPIS)[number]) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-2.5 sm:p-3 lg:p-3.5 min-w-0">
      <div className="text-[10px] sm:text-[11px] text-[#AAB7C7] truncate mb-1">{label}</div>
      <div className="flex items-baseline justify-between gap-2">
        <div className="text-[16px] sm:text-[18px] lg:text-[20px] font-semibold text-white tabular-nums">{value}</div>
        <div className={`text-[10px] sm:text-[11px] font-medium ${deltaColor} tabular-nums`}>{delta}</div>
      </div>
...
function MiniAreaChart() {
  return (
    <svg
      ...
      className="w-full h-[88px] sm:h-[120px] lg:h-[160px]"
      role="img"
...
function ActivityList() {
  const items = [
    { label: "تم اعتماد تقرير المبيعات", time: "قبل ٣ دقائق", tone: "cyan" },
    { label: "اكتمل سير أتمتة المتأخرات", time: "قبل ١٢ دقيقة", tone: "cyan" },
    { label: "عميل جديد: شركة الواحة", time: "قبل ساعة", tone: "cyan" },
  ];
  return (
    <ul className="space-y-2">
      {items.map((it) => (
        <li
          key={it.label}
          className="flex items-center gap-2.5 rounded-lg border border-white/[0.06] bg-white/[0.02] px-2.5 py-2 min-w-0"
        >
        ...
        <div className="p-3 sm:p-4 lg:p-5">
          <div className="flex items-center justify-between mb-3 lg:mb-4 min-w-0">
            ...
            <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/[0.03] border border-white/[0.06]">
              <svg viewBox="0 0 24 24" className="h-3 w-3 text-[#22D3EE]" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-2.5 lg:gap-3 mb-3 lg:mb-4">
            {KPIS.map((k) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-2.5 lg:gap-3">
            <div className="lg:col-span-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 lg:p-4 min-w-0">
            ...
            <div className="hidden lg:block lg:col-span-2 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 lg:p-4 min-w-0">
              <div className="flex items-center justify-between mb-2">
