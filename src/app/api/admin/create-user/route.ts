...
// Single top-level try/catch. ALL logic inside it. ALL supabase calls awaited.
// NO module-level state, NO top-level await, NO top-level createClient.
...
export async function POST(req: Request) {
  try {
    ...
    // ── service-role client (the ONLY client; no anon path) ───────────────
    const admin = createClient(SUPABASE_URL, SERVICE_KEY, {
      auth: { autoRefreshToken: false, persistSession: false },
    });

    // ── caller auth (await; service-role validates the JWT) ───────────────
    ...
    // ── parse body (await; outer catch handles malformed JSON) ────────────
    const body = (await req.json()) as Record<string, unknown>;
    console.log("[create-user] request parsed");

    // ── inline clean + validate ───────────────────────────────────────────
    ...
    // ── upsert profile (await; rollback auth user on failure) ─────────────
    ...
    // ── upsert employee (await; rollback auth user on failure) ────────────
